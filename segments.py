...
class Segments:
    ...
    def digest(self, number):
        ...
        for a in range(len(self.segments)):
            ...
            # Assume sh and sw are your divisors, representing height and width
            if sh != 0 and sw != 0:  # Ensure both sh and sw are not zero
                ratio = count / (sh * sw)
                if ratio > 0.5:  # 0.5 is a sensitivity measure
                    self.flags.append(a);
